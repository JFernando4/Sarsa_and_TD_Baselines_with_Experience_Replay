...
from Experiment_Engine.sarsa_zero_agent import SarsaZeroReturnFunction
...
class ExperienceReplayBuffer:
    ...
    def __init__(self, config, return_function):
        ...
        assert isinstance(return_function, SarsaZeroReturnFunction)
    ...
    def store_observation(self, observation):
        ...
        assert all(akey in observation.keys() for akey in ["reward", "action", "state", "terminate", "bprobabilities"])
    ...
    def get_data(self, update_function):
        ...
        sample_actions = self.action.data.take(bf_start + indices, axis=0, mode='wrap')
        ...
        if adjusted_batch_sz > 0:
            ...
            next_qvalues = update_function(next_state, reshape=False).reshape([adjusted_batch_sz, self.num_actions])
            next_reward = self.reward.data.take(next_time_step_indices, axis=0, mode='wrap')
            next_action = self.action.data.take(next_time_step_indices, axis=0, mode='wrap')
            next_action_bprob = self.bprobabilities.data.take(next_time_step_indices, axis=0, mode='wrap')
            ...
            estimated_returns[not_utd_batch_indices] = \
                self.return_function.batch_return_function(next_reward, next_action, next_qvalues,
                                                           next_action_bprob, next_termination, next_timeout,
                                                           adjusted_batch_sz)

            # self.estimated_return.data.put(indices=next_time_step_indices,
            #                                values=estimated_returns[not_utd_batch_indices], mode='wrap')
            # self.up_to_date.data.put(indices=next_time_step_indices, values=True, mode='wrap')
